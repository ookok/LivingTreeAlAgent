...
def test_ollama_connection():
    ...
    try:
        ...
        print(f"✅ Ollama API 连接成功: online={is_online}")
...
def test_agent_initialization():
    ...
    try:
        ...
        # 初始化Agent
        agent = HermesAgent(config, backend="ollama")
        print("✅ Agent 初始化成功")
        ...
        # 使用流式输出
        content = ""
        for chunk in agent.send_message(test_prompt):
            if chunk.delta:
                content += chunk.delta
                print(chunk.delta, end="")
            if chunk.error:
                print(f"\n❌ Agent 调用失败: {chunk.error}")
                return False
        print()
...
def main():
    ...
    if not config.ollama.default_model:
        config.ollama.default_model = "qwen3.6:27b"
        save_config(config)
        print("已设置默认模型: qwen3.6:27b")
