...
from .node_manager import EnterpriseNodeManager, get_enterprise_manager, EnterpriseNode
try:
    from livingtree.core.task_queue import TaskQueue, Task, TaskPriority
except ImportError:
    TaskQueue = Task = TaskPriority = None

def _lazy_get_intelligent_router():
    from .intelligent_router import get_intelligent_router as _gir
    return _gir()

def _lazy_get_cost_optimizer():
    from .intelligent_router import get_cost_optimizer as _gco
    return _gco()
...
class EnterpriseTaskScheduler:
    ...
    def _select_node(self, task: EnterpriseTask, nodes: List[EnterpriseNode]) -> Optional[EnterpriseNode]:
        ...
        # 使用智能任务路由器选择最佳节点
        router = _lazy_get_intelligent_router()
    ...
    def _complete_task(self, task: EnterpriseTask, result: Any):
        ...
        # 记录成本数据
        optimizer = _lazy_get_cost_optimizer()
        if task.assigned_node_id:
            # 计算实际成本（基于资源使用和执行时间）
