...
async def get_agent_data(launch_agent) -> Agent | None:
    ...
    from livingtree.tui.td.agents import read_agents, AgentReadError
...
def acp(
    ...
    identity = f"{command_name}.custom.livingtree.ai"
...
def serve(port: int, host: str, public_url: str | None = None) -> None:
    ...
    server = Server(
        sys.argv[0], host=host, port=port, title="LivingTree", public_url=public_url
    )
    set_process_title("toad serve")
    server.serve()
