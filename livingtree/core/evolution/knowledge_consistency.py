...
class ConsistencyChecker:
    ...
    def extract_key_facts(self, text: str) -> List[str]:
        ...
        proper_nouns = re.findall(
            r'[\u4e00-\u9fff]{2,}(?:公司|大学|国家|组织)|[A-Z][a-z]+(?:\s+[A-Z][a-z]+)+',
            text)
...
class MultiModelInferrer:
    ...
    async def infer_async(self, query: str, model_ids: List[str],
                          context: Optional[Dict] = None) -> List[ModelResponse]:
        tasks = []
        ...
        return [
            ...
            else ModelResponse(model_id="error", model_name="error",
                               content="", latency_ms=0, error=str(r))
            for r in results
        ]

    async def _call_model(self, model_id: str, query: str,
                          context: Optional[Dict]) -> ModelResponse:
        start_time = time.time()
        model_info = self._model_clients.get(model_id, {})

        if not model_info:
            return ModelResponse(model_id=model_id, model_name=model_id,
        ...
        try:
            ...
            return ModelResponse(model_id=model_id, model_name=model_name,
                                 content=content, latency_ms=latency_ms,
                                 confidence=0.8, key_facts=key_facts)

        except Exception as e:
            return ModelResponse(
                model_id=model_id,
                model_name=model_info.get("name", model_id),
                content="", latency_ms=(time.time() - start_time) * 1000,
                error=str(e))

    def infer_sync(self, query: str, model_ids: List[str],
                   context: Optional[Dict] = None) -> List[ModelResponse]:
        try:
        ...
        return loop.run_until_complete(
            self.infer_async(query, model_ids, context))
...
class VotingDecider:
    ...
    def decide(self, responses: List[ModelResponse],
               strategy: str = "majority") -> Tuple[str, List[str], List[str]]:
        ...
        return (self._majority_vote(valid) if strategy == "majority"
                else self._similarity_vote(valid))
        ...
        for i, r1 in enumerate(responses):
            ...
            for r2 in responses[i + 1:]:
                if r2.model_id in processed:
                    continue
                if self._checker.calculate_similarity(
                        r1.content, r2.content) >= self.similarity_threshold:
                    group.append(r2)
...
class KnowledgeConsistencyVerifier:
    def __init__(self, min_models: int = 2, consensus_threshold: float = 0.6,
        ...
        self._stats = {"total_verifications": 0, "full_consensus_count": 0,
                       "partial_consensus_count": 0, "conflict_count": 0}

    def register_model(self, model_id: str, model_name: str, client: Any):
        self._inferrer.register_model(model_id, model_name, client)

    def get_registered_models(self) -> List[Dict[str, str]]:
        return [{"id": mid, "name": info["name"]}
                for mid, info in self._inferrer._model_clients.items()]

    def verify(self, query: str, model_ids: Optional[List[str]] = None,
               context: Optional[Dict] = None,
               wait_for_all: bool = True) -> VerificationReport:
        start_time = time.time()
        ...
        if len(model_ids) < self.min_models:
            return VerificationReport(
                query=query, status=VerificationStatus.FAILED,
                result=None, all_responses=[],
                recommendation="注册的模型数量不足",
                metadata={"error": "min_models_not_met"})
        ...
        if len(valid) < self.min_models:
            return VerificationReport(
                query=query, status=VerificationStatus.FAILED,
                result=None, all_responses=responses,
                recommendation="有效模型响应不足",
                metadata={"error": "insufficient_responses"})
        ...
        return VerificationReport(
            ...
            result=ConsistencyResult(
                ...
                agreeing_models=result["agreeing"],
                conflicting_models=result["conflicting"],
                key_facts_agreement=result["fact_agreement"],
                final_answer=result["final_answer"],
                confidence=result["confidence"],
                disputed_facts=result["disputed_facts"],
                verification_time_ms=verification_time_ms,
                models_used=[r.model_id for r in valid]),
            all_responses=responses,
            recommendation=self._generate_recommendation(result),
            metadata={"models_count": len(valid),
                      "verification_time_ms": verification_time_ms})

    def _analyze_consistency(self, query: str,
                             responses: List[ModelResponse]) -> Dict:
        similarities = []
        for i, r1 in enumerate(responses):
            for r2 in responses[i + 1:]:
                similarities.append(
                    self._checker.calculate_similarity(r1.content, r2.content))

        avg_similarity = (sum(similarities) / len(similarities)
                          if similarities else 0)
        fact_agreement, disputed_facts = \
            self._checker.calculate_fact_agreement(responses)
        ...
        return {
            "level": level, "score": avg_similarity,
            "final_answer": final_answer, "agreeing": agreeing,
            "conflicting": conflicting, "fact_agreement": fact_agreement,
            "disputed_facts": disputed_facts, "confidence": confidence}
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            **self._stats,
            "consensus_rate": self._stats["full_consensus_count"] / total,
            "conflict_rate": self._stats["conflict_count"] / total,
        }
    ...
    def quick_check(self, query: str, answer: str) -> Dict:
        ...
        return {
            ...
            "estimated_confidence": min(len(key_facts) * 0.1 + 0.3, 0.9),
        }
