...
class ComplexityEstimator:
    def estimate(self, query: str, task_type: TaskType) -> Tuple[TaskComplexity, Dict]:
        ...
        complex_tasks = {TaskType.REASONING: 0.4, TaskType.MATHEMATICS: 0.5,
                         TaskType.CODE_GENERATION: 0.4, TaskType.ANALYSIS: 0.5,
                         TaskType.RESEARCH: 0.6}
        factors["task_score"] = complex_tasks.get(task_type, 0.2)

        total = (factors["length_score"] * 0.2 + factors["structure_score"] * 0.2 +
                 factors["domain_score"] * 0.3 + factors["task_score"] * 0.3)
...
class PerformanceTracker:
    def __init__(self, history_size: int = 1000):
        ...
        self._task_performance: Dict[str, Dict[TaskType, Dict]] = defaultdict(
            lambda: defaultdict(lambda: {"count": 0, "success": 0,
                                          "total_latency": 0, "quality_scores": []}))
        self._model_overall: Dict[str, Dict] = defaultdict(
            lambda: {"count": 0, "success": 0, "total_latency": 0})

    def record(self, model_id: str, task_type: TaskType, latency_ms: float,
    ...
    def get_stats(self) -> Dict:
        with self._lock:
            return {"tracked_models": len(self._model_overall),
                    "total_records": sum(m["count"] for m in self._model_overall.values())}
...
class AutoModelSelector:
    ...
    def register_model(self, model_id: str, model_name: str,
        ...
        strengths = [TaskType(s) if isinstance(s, str) else s
                     for s in capabilities.get("strengths", [])]
        weaknesses = [TaskType(w) if isinstance(w, str) else w
                      for w in capabilities.get("weaknesses", [])]
    ...
    def recommend(self, query: str, context: Optional[Dict] = None,
                  force_model: Optional[str] = None) -> ModelRecommendation:
        ...
        if force_model and force_model in self._models:
            ...
            return ModelRecommendation(
                ...
                fallback_models=self._get_fallback(force_model, task_type),
                estimated_latency_ms=primary.avg_latency_ms,
                estimated_cost=0,
                reasoning=f"强制使用{primary.model_name}",
                confidence=1.0)
    ...
    def _score_models(self, query: str, task_type: TaskType,
                      complexity: TaskComplexity) -> List[Tuple[str, float]]:
        candidates = []
        ...
        if self.prefer_free:
            free = [(mid, s) for mid, s in candidates
                    if self._models[mid].cost_per_1k_tokens == 0]
            if free: candidates = free

        valid = [(mid, s) for mid, s in candidates
                 if self._models[mid].avg_latency_ms <= self.latency_budget_ms]
        return valid if valid else candidates[:5]

    def _calculate_score(self, model: ModelCapability, task_type: TaskType,
    ...
    def _default_recommendation(self) -> ModelRecommendation:
        ...
        return ModelRecommendation(
            primary_model=primary,
            fallback_models=list(self._models.values())[1:4],
            estimated_latency_ms=primary.avg_latency_ms,
            estimated_cost=0, reasoning="默认模型", confidence=0.3)
    ...
    def record_result(self, model_id: str, query: str, latency_ms: float,
                      success: bool, quality_score: Optional[float] = None):
    ...
    def get_model_list(self) -> List[Dict]:
        return [{"id": m.model_id, "name": m.model_name,
                 "strengths": [t.value for t in m.strengths]}
