...
def _get_system_defaults() -> Dict[str, str]:
    ...
    try:
        from business.config_provider import get_ollama_url, get_l3_model
        return {
...
class IntentActionBridge:
    ...
    def _report_to_evolution(self, intent: Intent, result: ActionResult, execution_time: float):
        ...
        """
        try:
            from business.evolution_engine.bridge import (
            ...
            if bridge is None:
                # 延迟创建（第一次使用时）
                try:
                    from business.evolution_engine.bridge import create_bridge
