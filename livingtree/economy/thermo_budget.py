...
class ThermodynamicBudget:
    ...
    def evaluate(
        ...
        Physically motivated decision rules:
          ...
          2. Entropy-driven model tier: high entropy → explore cheap models
             low entropy → exploit quality models
        ...
        # Entropy-driven model tier
        if state.entropy > 0.7:
            model_tier = "flash"       # High entropy → conservative, use cheap
        elif state.entropy > 0.3:
            model_tier = "pro"         # Moderate → balanced
        else:
            model_tier = "ultra"       # Low entropy → confident, use best
        ...
        if predicted_quality < 0.3 and model_tier != "flash":
            model_tier = "pro"  # Don't waste ultra on likely-failing tasks
