#!/usr/bin/env python3
"""
LivingTreeAI Phase 2 - Agent 通信协议
定义多智能体之间的通信格式和交互模式
"""

from typing import Dict, List, Optional, Any
from dataclasses import dataclass, field
from enum import Enum
from datetime import datetime
import json


class MessageType(Enum):
    """消息类型"""
    REQUEST = "request"           # 请求
    RESPONSE = "response"        # 响应
    BROADCAST = "broadcast"      # 广播
    NOTIFICATION = "notification"  # 通知
    HEARTBEAT = "heartbeat"     # 心跳


class MessagePriority(Enum):
    """消息优先级"""
    LOW = 0
    NORMAL = 1
    HIGH = 2
    URGENT = 3


@dataclass
class AgentMessage:
    """智能体消息"""
    msg_id: str
    msg_type: MessageType
    sender: str
    receiver: str  # 空字符串表示广播
    action: str
    payload: Dict[str, Any]
    priority: MessagePriority = MessagePriority.NORMAL
    timestamp: float = field(default_factory=datetime.now().timestamp)
    reply_to: Optional[str] = None  # 关联的消息ID
    metadata: Dict[str, Any] = field(default_factory=dict)


@dataclass
class Conversation:
    """对话记录"""
    conv_id: str
    participants: List[str]
    messages: List[AgentMessage] = field(default_factory=list)
    created_at: float = field(default_factory=datetime.now().timestamp)
    updated_at: float = field(default_factory=datetime.now().timestamp)


class AgentProtocol:
    """
    Agent 通信协议
    定义多智能体之间的通信格式和交互模式
    """
    
    def __init__(self, agent_id: str):
        self.agent_id = agent_id
        self.conversations: Dict[str, Conversation] = {}
        self.outbox: List[AgentMessage] = []
        self.inbox: List[AgentMessage] = []
        self.subscriptions: Dict[str, List[str]] = {}  # topic -> agent_ids
    
    # ==================== 消息构建 ====================
    
    def create_request(self, receiver: str, action: str,
                      payload: Dict[str, Any] = None) -> AgentMessage:
        """创建请求消息"""
        import uuid
        
        msg = AgentMessage(
            msg_id=str(uuid.uuid4())[:12],
            msg_type=MessageType.REQUEST,
            sender=self.agent_id,
            receiver=receiver,
            action=action,
            payload=payload or {}
        )
        
        self.outbox.append(msg)
        return msg
    
    def create_response(self, original_msg: AgentMessage,
                       success: bool, result: Any = None,
                       error: str = None) -> AgentMessage:
        """创建响应消息"""
        import uuid
        
        payload = {
            'success': success,
            'result': result
        }
        if error:
            payload['error'] = error
        
        msg = AgentMessage(
            msg_id=str(uuid.uuid4())[:12],
            msg_type=MessageType.RESPONSE,
            sender=self.agent_id,
            receiver=original_msg.sender,
            action=f"{original_msg.action}_response",
            payload=payload,
            reply_to=original_msg.msg_id
        )
        
        self.outbox.append(msg)
        return msg
    
    def create_broadcast(self, action: str, payload: Dict[str, Any] = None,
                       topic: str = None) -> AgentMessage:
        """创建广播消息"""
        import uuid
        
        msg = AgentMessage(
            msg_id=str(uuid.uuid4())[:12],
            msg_type=MessageType.BROADCAST,
            sender=self.agent_id,
            receiver="",  # 空表示广播
            action=action,
            payload=payload or {},
            metadata={'topic': topic} if topic else {}
        )
        
        self.outbox.append(msg)
        return msg
    
    def create_notification(self, receiver: str, action: str,
                           payload: Dict[str, Any] = None) -> AgentMessage:
        """创建通知消息"""
        import uuid
        
        msg = AgentMessage(
            msg_id=str(uuid.uuid4())[:12],
            msg_type=MessageType.NOTIFICATION,
            sender=self.agent_id,
            receiver=receiver,
            action=action,
            payload=payload or {}
        )
        
        self.outbox.append(msg)
        return msg
    
    def create_heartbeat(self) -> AgentMessage:
        """创建心跳消息"""
        return self.create_broadcast("heartbeat", {
            'agent_id': self.agent_id,
            'timestamp': datetime.now().timestamp
        }, topic="heartbeat")
    
    # ==================== 消息处理 ====================
    
    def send_message(self, msg: AgentMessage) -> bool:
        """发送消息"""
        # 添加到发送队列
        self.outbox.append(msg)
        return True
    
    def receive_message(self, msg: AgentMessage) -> None:
        """接收消息"""
        self.inbox.append(msg)
        
        # 如果是回复，更新对话
        if msg.reply_to:
            for conv in self.conversations.values():
                for m in conv.messages:
                    if m.msg_id == msg.reply_to:
                        m.metadata['reply'] = msg
    
    def process_inbox(self) -> List[AgentMessage]:
        """处理收件箱"""
        messages = self.inbox.copy()
        self.inbox.clear()
        return messages
    
    def get_outbox(self) -> List[AgentMessage]:
        """获取发送队列"""
        outbox = self.outbox.copy()
        self.outbox.clear()
        return outbox
    
    # ==================== 对话管理 ====================
    
    def start_conversation(self, conv_id: str, participants: List[str]) -> Conversation:
        """开始对话"""
        conv = Conversation(
            conv_id=conv_id,
            participants=participants
        )
        self.conversations[conv_id] = conv
        return conv
    
    def add_to_conversation(self, conv_id: str, msg: AgentMessage) -> bool:
        """添加消息到对话"""
        if conv_id not in self.conversations:
            return False
        
        conv = self.conversations[conv_id]
        conv.messages.append(msg)
        conv.updated_at = datetime.now().timestamp
        return True
    
    def get_conversation(self, conv_id: str) -> Optional[Conversation]:
        """获取对话"""
        return self.conversations.get(conv_id)
    
    def get_conversation_history(self, conv_id: str,
                                 limit: int = 50) -> List[AgentMessage]:
        """获取对话历史"""
        conv = self.conversations.get(conv_id)
        if not conv:
            return []
        
        return conv.messages[-limit:]
    
    # ==================== 订阅机制 ====================
    
    def subscribe(self, topic: str) -> None:
        """订阅主题"""
        if topic not in self.subscriptions:
            self.subscriptions[topic] = []
        
        if self.agent_id not in self.subscriptions[topic]:
            self.subscriptions[topic].append(self.agent_id)
    
    def unsubscribe(self, topic: str) -> None:
        """取消订阅"""
        if topic in self.subscriptions:
            if self.agent_id in self.subscriptions[topic]:
                self.subscriptions[topic].remove(self.agent_id)
    
    def get_subscribers(self, topic: str) -> List[str]:
        """获取主题订阅者"""
        return self.subscriptions.get(topic, [])
    
    # ==================== 序列化 ====================
    
    def serialize_message(self, msg: AgentMessage) -> str:
        """序列化消息为JSON"""
        return json.dumps({
            'msg_id': msg.msg_id,
            'msg_type': msg.msg_type.value,
            'sender': msg.sender,
            'receiver': msg.receiver,
            'action': msg.action,
            'payload': msg.payload,
            'priority': msg.priority.value,
            'timestamp': msg.timestamp,
            'reply_to': msg.reply_to,
            'metadata': msg.metadata
        })
    
    def deserialize_message(self, data: str) -> AgentMessage:
        """从JSON反序列化消息"""
        obj = json.loads(data)
        return AgentMessage(
            msg_id=obj['msg_id'],
            msg_type=MessageType(obj['msg_type']),
            sender=obj['sender'],
            receiver=obj['receiver'],
            action=obj['action'],
            payload=obj['payload'],
            priority=MessagePriority(obj.get('priority', 1)),
            timestamp=obj.get('timestamp', datetime.now().timestamp),
            reply_to=obj.get('reply_to'),
            metadata=obj.get('metadata', {})
        )


class ProtocolRegistry:
    """
    协议注册表
    管理所有智能体的协议实例
    """
    
    def __init__(self):
        self.protocols: Dict[str, AgentProtocol] = {}
    
    def register(self, agent_id: str) -> AgentProtocol:
        """注册智能体协议"""
        if agent_id not in self.protocols:
            self.protocols[agent_id] = AgentProtocol(agent_id)
        return self.protocols[agent_id]
    
    def get_protocol(self, agent_id: str) -> Optional[AgentProtocol]:
        """获取智能体协议"""
        return self.protocols.get(agent_id)
    
    def get_all_protocols(self) -> Dict[str, AgentProtocol]:
        """获取所有协议"""
        return self.protocols.copy()
    
    def route_message(self, msg: AgentMessage) -> bool:
        """路由消息"""
        # 如果是广播，发送到所有订阅者
        if msg.msg_type == MessageType.BROADCAST:
            topic = msg.metadata.get('topic')
            if topic:
                subscribers = []
                for proto in self.protocols.values():
                    if topic in proto.subscriptions:
                        subscribers.extend(proto.subscriptions[topic])
                
                # 发送到所有订阅者
                for subscriber_id in set(subscribers):
                    proto = self.protocols.get(subscriber_id)
                    if proto and subscriber_id != msg.sender:
                        proto.receive_message(msg)
            return True
        
        # 如果是定向消息
        receiver_proto = self.protocols.get(msg.receiver)
        if receiver_proto:
            receiver_proto.receive_message(msg)
            return True
        
        return False


# ==================== 全局实例 ====================

_protocol_registry: Optional[ProtocolRegistry] = None


def get_protocol_registry() -> ProtocolRegistry:
    """获取全局协议注册表"""
    global _protocol_registry
    if _protocol_registry is None:
        _protocol_registry = ProtocolRegistry()
    return _protocol_registry


# ==================== CLI ====================

def main():
    """命令行工具"""
    import argparse
    
    parser = argparse.ArgumentParser(description='Agent 通信协议')
    parser.add_argument('--register', '-r', help='注册智能体')
    parser.add_argument('--send', '-s', nargs=3, metavar=('TO', 'ACTION', 'PAYLOAD'),
                       help='发送消息')
    parser.add_argument('--broadcast', '-b', nargs=2, metavar=('ACTION', 'PAYLOAD'),
                       help='广播消息')
    
    args = parser.parse_args()
    
    registry = get_protocol_registry()
    
    if args.register:
        proto = registry.register(args.register)
        print(f"注册智能体: {args.register}")
    
    if args.send:
        to, action, payload = args.send
        if to not in registry.protocols:
            registry.register(to)
        registry.get_protocol(args.register)
        
        from_agent = args.register
        proto = registry.get_protocol(from_agent)
        msg = proto.create_request(to, action, {'data': payload})
        registry.route_message(msg)
        print(f"发送消息: {msg.msg_id} -> {to}")
    
    if args.broadcast:
        action, payload = args.broadcast
        proto = registry.get_protocol(args.register)
        msg = proto.create_broadcast(action, {'data': payload})
        registry.route_message(msg)
        print(f"广播消息: {msg.msg_id}")


if __name__ == "__main__":
    main()
