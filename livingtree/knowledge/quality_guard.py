...
__all__ = ["KnowledgeQualityTest", "run_quality_tests", "QUALITY_TEMPLATES", "QualityScore", "ContentLabel", "ContentQuality"]

_SAFE_BUILTINS = {
    "len": len, "max": max, "min": min, "set": set, "sum": sum,
...
def _eval_condition(condition: str, text: str) -> bool:
    ns = dict(_SAFE_BUILTINS, text=text)
...
def run_quality_tests(text: str, domain: str = "general", templates: list = None) -> dict:
    ...
    for test in tests:
        if test.domain not in (domain, "general"):
            continue
        try:
            result = _eval_condition(test.condition, text)
        except Exception:
            result = True
