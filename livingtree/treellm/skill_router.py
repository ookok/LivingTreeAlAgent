"""SkillRouter — Pure Python skill routing for LLM agents.

Inspired by SkillRouter (arxiv 2603.22455). Routes user queries to:
  1. Best LLM provider (replaces TinyClassifier)
  2. Most relevant system tools (from 21 tools + 8 roles)
  3. Best expert role for the task

Uses full-text TF-IDF retrieval + lightweight linear reranker.
Self-learns from success/failure feedback per routing decision.
No heavy deps: numpy + scikit-learn (already in project).
"""
