#!/usr/bin/env python3
"""迁移 core/agent 到 client/src/business/agent"""
import os
import shutil
import re

SRC_DIR = "core/agent"
DST_DIR = "client/src/business/agent"
...
def update_internal_imports():
    ...
    for filename in os.listdir(DST_DIR):
        ...
        # 替换 from core.agent 为 from client.src.business.agent
        pattern = r'from\s+core\.agent\s+import'
        replacement = 'from client.src.business.agent import'
...
def update_external_imports():
    """更新外部文件中的导入引用"""
    # 只更新真正引用 core.agent (目录) 的文件
    external_files = [
        "docs/API_REFERENCE.md",
        "client/src/presentation/panels/lobe_layout/skill_binding.py"
    ]
    ...
    for filepath in external_files:
        ...
        # 替换 from core.agent.orchestration_viewer 为 from client.src.business.agent.orchestration_viewer
        pattern = r'from\s+core\.agent\.orchestration_viewer\s+import'
        replacement = 'from client.src.business.agent.orchestration_viewer import'
...
if __name__ == "__main__":
    print("=" * 60)
    print("Migrating core/agent to client/src/business/")
    print("=" * 60)
