...
# ═══════════════════════════════════════════════════════════════════
# ParliamentSession — structured multi-role deliberative protocol
# ═══════════════════════════════════════════════════════════════════
...
class ParliamentSession:
    @staticmethod
    def default_roles() -> list[ParliamentRole]:
        return [
            ParliamentRole("analyst", "数据驱动的理性分析者", "逻辑分析、数据推理、因果推断"),
            ParliamentRole("critic", "严格的质量审查者", "找出逻辑漏洞、边界情况、隐含假设"),
            ParliamentRole("synthesizer", "多方视角的综合者", "整合不同观点、发现共识、构建框架"),
            ParliamentRole("innovator", "创造性思考者", "跳出框框、提出新思路、挑战常规"),
            ParliamentRole("pragmatist", "务实的执行者", "关注可行性、成本收益、实施路径"),
        ]

    async def deliberate(self, question: str, chat_fn=None,
                         roles: list[str] = None, max_rounds: int = 3) -> Verdict:
        t0 = time.time()
        agents = [r for r in self.default_roles() if not roles or r.name in roles][:5]
        if len(agents) < 2:
            agents = self.default_roles()[:3]

        verdict = Verdict(question=question)
        positions = await self._gather_positions(question, agents, chat_fn)

        for round_num in range(1, max_rounds + 1):
            critiques = await self._cross_examine(positions, agents, chat_fn)
            positions = await self._revise(positions, critiques, agents, chat_fn, question)
            verdict.rounds = round_num

        votes = await self._gather_votes(positions, agents, chat_fn)
        self._tally(verdict, votes, positions, agents)
        verdict.duration_ms = (time.time() - t0) * 1000
        logger.info(
            f"ParliamentSession: {len(agents)} agents, {verdict.rounds} rounds, "
            f"consensus={verdict.consensus_score:.2f}"
        )
        return verdict

    async def _gather_positions(self, question: str, agents: list[ParliamentRole],
                                 chat_fn) -> list[Position]:
        ...
        return await asyncio.gather(*[_ask(a) for a in agents])

    async def _cross_examine(self, positions: list[Position],
                              agents: list[ParliamentRole], chat_fn) -> list[Critique]:
        ...
        return critiques[:len(agents) * 2]

    async def _revise(self, positions: list[Position], critiques: list[Critique],
                      agents: list[ParliamentRole], chat_fn, question: str) -> list[Position]:
        ...
        return await asyncio.gather(*[_revise(a, pos) for a, pos in zip(agents, positions)])

    async def _gather_votes(self, positions: list[Position],
                             agents: list[ParliamentRole], chat_fn) -> list[Vote]:
        votes = []
        for agent in agents:
    ...
    def _tally(self, verdict: Verdict, votes: list[Vote],
               positions: list[Position], agents: list[ParliamentRole]) -> None:
...
_parliament: ParliamentSession | None = None
_parliament_lock = threading.Lock()


def get_parliament() -> ParliamentSession:
    global _parliament
    if _parliament is None:
        with _parliament_lock:
            if _parliament is None:
                _parliament = ParliamentSession()
    return _parliament
