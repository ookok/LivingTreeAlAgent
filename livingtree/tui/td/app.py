...
import toad
from livingtree.tui.td.db import DB
from livingtree.tui.td.settings import Schema, Settings
from livingtree.tui.td.agent_schema import Agent as AgentData
from livingtree.tui.td import messages
from livingtree.tui.td.settings_schema import SCHEMA
from livingtree.tui.td.version import VersionMeta
from livingtree.tui.td import paths
from livingtree.tui.td import atomic
from livingtree.tui.td.session_tracker import SessionTracker, SessionDetails

if TYPE_CHECKING:
    from livingtree.tui.td.screens.main import MainScreen
    from livingtree.tui.td.screens.settings import SettingsScreen
    from livingtree.tui.td.screens.store import StoreScreen
    from livingtree.tui.td.screens.sessions import SessionsScreen
    from livingtree.tui.td.db import DB
...
class ToadApp(App, inherit_bindings=False):
    ...
    terminal_title: var[str] = var("LivingTree")
    ...
    def settings_path(self) -> Path:
        return paths.get_config() / "livingtree.json"

    @property
    def db_path(self) -> Path:
        return paths.get_state() / "livingtree.db"
    ...
    def version(self) -> str:
        """Version of the app."""
        from livingtree.tui.td import get_version
    ...
    def anon_id(self) -> str:
        ...
        if not (anon_id := self.settings.get("anon_id", str, expand=False)):
            ...
            self._save_settings()
            self.call_later(self.capture_event, "livingtree-install")
        return anon_id
    ...
    async def capture_event(self, event_name: str, **properties: Any) -> None:
        ...
        event_properties = {
            "livingtree_version": self.version,
    ...
    def system_notify(
        ...
        notification.application_name = "🌳 Toad" if toad.os == "macos" else "LivingTree"
    ...
    async def on_mount(self) -> None:
        self.capture_event("livingtree-run")
    ...
    def set_process_title(self) -> None:
        try:
            ...
            setproctitle.setproctitle("LivingTree")
        except Exception:
            pass
    ...
    async def run_version_check(self) -> None:
        ...
        from livingtree.tui.td.version import check_version, VersionCheckFailed
    ...
    async def launch_agent(
        ...
        *,
        ...
        from livingtree.tui.td.agents import read_agents
