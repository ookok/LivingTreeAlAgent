...
# 懒加载，避免循环导入
# 使用方式：
#   from . import SignalBus, SignalType, A2AEigenFluxBridge
...
def get_distributed_modules():
    ...
    获取分布式集群模块
    """
    from .distributed_signal_bus import (
        ClusterConfig,
        NodeState,
        NodeInfo,
        ClusterMessageType,
...
def SignalBus(*args, **kwargs):
    ...
    from .eigenflux import SignalBus as _SignalBus
    return _SignalBus(*args, **kwargs)


def SignalType(*args, **kwargs):
    """EigenFlux 信号类型枚举"""
    from .eigenflux import SignalType as _SignalType
    return _SignalType(*args, **kwargs)


def A2AEigenFluxBridge(*args, **kwargs):
    """A2A 与 EigenFlux 桥接器"""
    from .eigenflux import A2AEigenFluxBridge as _Bridge
    return _Bridge(*args, **kwargs)


def SemanticEmbedder(*args, **kwargs):
    """语义嵌入生成器"""
    from .semantic_embedder import SemanticEmbedder as _Embedder
    return _Embedder(*args, **kwargs)


def InteropGateway(*args, **kwargs):
    """跨框架互操作网关"""
    from .interop_adapters import InteropGateway as _Gateway
    return _Gateway(*args, **kwargs)


def DistributedSignalBus(*args, **kwargs):
    """分布式信号总线"""
    from .distributed_signal_bus import DistributedSignalBus as _Bus
