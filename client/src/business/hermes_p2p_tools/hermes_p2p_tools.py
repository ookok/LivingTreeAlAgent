...
返回：配置项列表、缺失项、建议""",
    ...
    def p2p_check_config(context: Dict, feature: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            from business.config_missing_detector import get_config_detector
...
返回：更新结果""",
    ...
    def p2p_update_config(context: Dict, key: str, value: str) -> Dict:
        """更新配置"""
        try:
            from business.config import load_config, save_config
            import json
    ...
    def p2p_validate_config(context: Dict, config_type: str, value: str) -> Dict:
        ...
        try:
            if config_type == "api_key":
                # 验证API Key
                from business.key_management import get_key_manager
                km = get_key_manager()
