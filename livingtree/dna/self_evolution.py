...
class AutonomousCodeEvolution:
    ...
    def _commit_to_github(self, mutation: Mutation) -> bool:
        ...
        try:
            ...
            asyncio.run(git(f"checkout -b {branch}"))

            # Write mutation
            target_path.write_text(mutation.mutated, "utf-8")
            asyncio.run(git(f"add \"{target_path}\""))
            asyncio.run(git(f"commit -m \"evolved: {mutation.description}\""))

            # Push
            asyncio.run(git(f"push origin {branch}"))

            # Create PR
            asyncio.run(gh(
                f"pr create --title \"Auto-evolution gen{self._generation}\" "
                f"--body \"Auto-generated by CodeEvolution. "
